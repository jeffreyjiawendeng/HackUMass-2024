...
class BrightSide:
    def __init__(self, root):
        ...
        self.face_haar_cascade = cv2.CascadeClassifier(haar_file)
    ...
    def setup_ui(self):
        self.main_frame = tk.Frame(self.root, bg="#FFE5B4", padx=10, pady=10)  
        self.main_frame.pack(fill="both", expand=True)

        self.canvas = tk.Canvas(self.main_frame, width=230, height=200)  
        self.canvas.pack(pady=5)

        self.encouragement_text = tk.Label(self.main_frame, text="Stay positive!", font=("Segoe UI", 12), bg="#FFE5B4", fg="#5A4E35", pady=5, wraplength=230)
        self.encouragement_text.pack(pady=5, padx=5)

        self.exit_button_canvas = tk.Canvas(self.main_frame, width=100, height=100, bg="#FFE5B4", bd=0, highlightthickness=0)
    ...
    def update_frame(self):
        ...
        for (x, y, w, h) in faces_detected:
            cv2.rectangle(test_img, (x, y), (x + w, y + h), (180, 229, 255), thickness=7)
            ...
            cv2.putText(test_img, predicted_emotion, (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (162, 154, 255), 2)
    ...
    def resize_frame(self, frame, target_width, target_height):
        ...
        padded_frame = cv2.copyMakeBorder(resized_frame, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT, value=(180, 229, 255))
    ...
    def check_mood(self):
        ...
        if max_emotion in encouragement_data:
            message = random.choice(encouragement_data[max_emotion])
